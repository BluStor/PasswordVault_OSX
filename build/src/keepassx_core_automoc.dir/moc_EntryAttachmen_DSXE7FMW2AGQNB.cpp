/****************************************************************************
** Meta object code from reading C++ file 'EntryAttachments.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.8.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../src/core/EntryAttachments.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'EntryAttachments.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.8.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_EntryAttachments_t {
    QByteArrayData data[11];
    char stringdata0[110];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_EntryAttachments_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_EntryAttachments_t qt_meta_stringdata_EntryAttachments = {
    {
QT_MOC_LITERAL(0, 0, 16), // "EntryAttachments"
QT_MOC_LITERAL(1, 17, 8), // "modified"
QT_MOC_LITERAL(2, 26, 0), // ""
QT_MOC_LITERAL(3, 27, 11), // "keyModified"
QT_MOC_LITERAL(4, 39, 3), // "key"
QT_MOC_LITERAL(5, 43, 14), // "aboutToBeAdded"
QT_MOC_LITERAL(6, 58, 5), // "added"
QT_MOC_LITERAL(7, 64, 16), // "aboutToBeRemoved"
QT_MOC_LITERAL(8, 81, 7), // "removed"
QT_MOC_LITERAL(9, 89, 14), // "aboutToBeReset"
QT_MOC_LITERAL(10, 104, 5) // "reset"

    },
    "EntryAttachments\0modified\0\0keyModified\0"
    "key\0aboutToBeAdded\0added\0aboutToBeRemoved\0"
    "removed\0aboutToBeReset\0reset"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_EntryAttachments[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       8,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       8,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   54,    2, 0x06 /* Public */,
       3,    1,   55,    2, 0x06 /* Public */,
       5,    1,   58,    2, 0x06 /* Public */,
       6,    1,   61,    2, 0x06 /* Public */,
       7,    1,   64,    2, 0x06 /* Public */,
       8,    1,   67,    2, 0x06 /* Public */,
       9,    0,   70,    2, 0x06 /* Public */,
      10,    0,   71,    2, 0x06 /* Public */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void, QMetaType::QString,    4,
    QMetaType::Void, QMetaType::QString,    4,
    QMetaType::Void, QMetaType::QString,    4,
    QMetaType::Void, QMetaType::QString,    4,
    QMetaType::Void, QMetaType::QString,    4,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void EntryAttachments::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        EntryAttachments *_t = static_cast<EntryAttachments *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->modified(); break;
        case 1: _t->keyModified((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 2: _t->aboutToBeAdded((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 3: _t->added((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 4: _t->aboutToBeRemoved((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 5: _t->removed((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 6: _t->aboutToBeReset(); break;
        case 7: _t->reset(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (EntryAttachments::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&EntryAttachments::modified)) {
                *result = 0;
                return;
            }
        }
        {
            typedef void (EntryAttachments::*_t)(const QString & );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&EntryAttachments::keyModified)) {
                *result = 1;
                return;
            }
        }
        {
            typedef void (EntryAttachments::*_t)(const QString & );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&EntryAttachments::aboutToBeAdded)) {
                *result = 2;
                return;
            }
        }
        {
            typedef void (EntryAttachments::*_t)(const QString & );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&EntryAttachments::added)) {
                *result = 3;
                return;
            }
        }
        {
            typedef void (EntryAttachments::*_t)(const QString & );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&EntryAttachments::aboutToBeRemoved)) {
                *result = 4;
                return;
            }
        }
        {
            typedef void (EntryAttachments::*_t)(const QString & );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&EntryAttachments::removed)) {
                *result = 5;
                return;
            }
        }
        {
            typedef void (EntryAttachments::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&EntryAttachments::aboutToBeReset)) {
                *result = 6;
                return;
            }
        }
        {
            typedef void (EntryAttachments::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&EntryAttachments::reset)) {
                *result = 7;
                return;
            }
        }
    }
}

const QMetaObject EntryAttachments::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_EntryAttachments.data,
      qt_meta_data_EntryAttachments,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *EntryAttachments::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *EntryAttachments::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_EntryAttachments.stringdata0))
        return static_cast<void*>(const_cast< EntryAttachments*>(this));
    return QObject::qt_metacast(_clname);
}

int EntryAttachments::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 8)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 8;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 8)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 8;
    }
    return _id;
}

// SIGNAL 0
void EntryAttachments::modified()
{
    QMetaObject::activate(this, &staticMetaObject, 0, Q_NULLPTR);
}

// SIGNAL 1
void EntryAttachments::keyModified(const QString & _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void EntryAttachments::aboutToBeAdded(const QString & _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}

// SIGNAL 3
void EntryAttachments::added(const QString & _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 3, _a);
}

// SIGNAL 4
void EntryAttachments::aboutToBeRemoved(const QString & _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 4, _a);
}

// SIGNAL 5
void EntryAttachments::removed(const QString & _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 5, _a);
}

// SIGNAL 6
void EntryAttachments::aboutToBeReset()
{
    QMetaObject::activate(this, &staticMetaObject, 6, Q_NULLPTR);
}

// SIGNAL 7
void EntryAttachments::reset()
{
    QMetaObject::activate(this, &staticMetaObject, 7, Q_NULLPTR);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
